  ...
  extends LoaderManager
{
    ...
    for (int j = 0;; str++)
    {
      if (j >= this.mLoaders.size())
      {
        ...
        for (int i = 0;; i++)
        {
          ...
          localObject = (LoaderInfo)this.mInactiveLoaders.valueAt(i);
          paramPrintWriter.print(paramString);
          paramPrintWriter.print("  #");
          paramPrintWriter.print(this.mInactiveLoaders.keyAt(i));
          paramPrintWriter.print(": ");
          paramPrintWriter.println(((LoaderInfo)localObject).toString());
          ((LoaderInfo)localObject).dump(str, paramFileDescriptor, paramPrintWriter, paramArrayOfString);
        }
      }
      LoaderInfo localLoaderInfo = (LoaderInfo)this.mLoaders.valueAt(str);
      paramPrintWriter.print(paramString);
      paramPrintWriter.print("  #");
      paramPrintWriter.print(this.mLoaders.keyAt(str));
      paramPrintWriter.print(": ");
  ...
  public <D> Loader<D> restartLoader(int paramInt, Bundle paramBundle, LoaderManager.LoaderCallbacks<D> paramLoaderCallbacks)
  {
    if (!this.mCreatingLoader)
    {
      ...
      if (localLoaderInfo != null)
      {
        localObject = (LoaderInfo)this.mInactiveLoaders.get(paramInt);
        if (localObject == null)
        {
          if (DEBUG) {
            Log.v("LoaderManager", "  Making last loader inactive: " + localLoaderInfo);
          }
          localLoaderInfo.mLoader.abandon();
          this.mInactiveLoaders.put(paramInt, localLoaderInfo);
        }
        else if (!localLoaderInfo.mHaveData)
        {
          if (localLoaderInfo.mStarted)
          {
            ...
            {
              if (DEBUG) {
                Log.v("LoaderManager", "  Removing pending loader: " + localLoaderInfo.mPendingLoader);
              }
              localLoaderInfo.mPendingLoader.destroy();
              localLoaderInfo.mPendingLoader = null;
            }
            if (DEBUG) {
              Log.v("LoaderManager", "  Enqueuing as new pending loader");
            }
            localLoaderInfo.mPendingLoader = createLoader(paramInt, paramBundle, paramLoaderCallbacks);
        ...
        {
          if (DEBUG) {
            Log.v("LoaderManager", "  Removing last inactive loader: " + localLoaderInfo);
          }
          ((LoaderInfo)localObject).mDeliveredData = false;
          ((LoaderInfo)localObject).destroy();
          localLoaderInfo.mLoader.abandon();
          this.mInactiveLoaders.put(paramInt, localLoaderInfo);
        }
      }
      Object localObject = createAndInstallLoader(paramInt, paramBundle, paramLoaderCallbacks).mLoader;
      label358:
      return (Loader<D>)localObject;
    }
    throw new IllegalStateException("Called while creating a loader");
...
/* Location:              E:\classes_dex2jar.jar!\android\support\v4\app\LoaderManagerImpl.class
