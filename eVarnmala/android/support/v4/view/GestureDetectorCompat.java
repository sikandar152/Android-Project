      ...
      for (int i1 = 0;; f10++)
      {
        ...
        if (i1 >= i)
        {
            ...
            if (!this.mIsDoubleTapping)
            {
              if (!this.mInLongPress)
              {
                if (!this.mAlwaysInTapRegion)
                {
                  ...
                  f3 = VelocityTrackerCompat.getYVelocity(localVelocityTracker, m);
                  f7 = VelocityTrackerCompat.getXVelocity(localVelocityTracker, m);
                  if ((Math.abs(f3) > this.mMinimumFlingVelocity) || (Math.abs(f7) > this.mMinimumFlingVelocity)) {
                    bool1 = this.mListener.onFling(this.mCurrentDownEvent, paramMotionEvent, f7, f3);
                  }
                }
                else
                {
          ...
          case 2: 
            if (!this.mInLongPress)
            {
              f3 = this.mLastFocusX - localMotionEvent;
              ...
              if (!this.mIsDoubleTapping)
              {
                if (!this.mAlwaysInTapRegion)
                {
                  if ((Math.abs(f3) >= 1.0F) || (Math.abs(f8) >= 1.0F))
                  {
                    ...
                    this.mLastFocusX = localMotionEvent;
                    this.mLastFocusY = f7;
                  }
                }
                else
                {
                  i1 = (int)(localMotionEvent - this.mDownFocusX);
                  n = (int)(f7 - this.mDownFocusY);
                  n = i1 * i1 + n * n;
          ...
          case 5: 
            this.mLastFocusX = localMotionEvent;
            this.mDownFocusX = localMotionEvent;
            this.mLastFocusY = f7;
            this.mDownFocusY = f7;
            cancelTaps();
            break;
          case 6: 
            ...
            float f9 = MotionEventCompat.getPointerId(paramMotionEvent, j);
            f7 = VelocityTrackerCompat.getXVelocity(this.mVelocityTracker, f9);
            f10 = VelocityTrackerCompat.getYVelocity(this.mVelocityTracker, f9);
            f9 = 0;
            while (f9 < f3)
            {
              if (f9 != j)
              {
                n = MotionEventCompat.getPointerId(paramMotionEvent, f9);
                if (f7 * VelocityTrackerCompat.getXVelocity(this.mVelocityTracker, n) + f10 * VelocityTrackerCompat.getYVelocity(this.mVelocityTracker, n) < 0.0F) {}
              }
              else
              {
                f9++;
...
/* Location:              E:\classes_dex2jar.jar!\android\support\v4\view\GestureDetectorCompat.class
